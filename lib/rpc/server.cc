...
struct server::impl {
    ...
    impl(server *parent, uint16_t port)
        : parent_(parent),
          ...
          suppress_exceptions_(false) {}
#endif
    impl(server *parent, std::string const &name)
        : parent_(parent),
          io_(),
...
server::server(std::string const &name)    
    : pimpl(new server::impl(this, name)), 
    disp_(std::make_shared<dispatcher>()) {
    LOG_INFO("Created server on local domain socket:{}", name);
    pimpl->start_accept();
}
...
void server::run(session_end_callback_t session_end_callback)
{
    session_end_callback_ = session_end_callback;
    pimpl->io_.run();
}
...
void server::close_session(std::shared_ptr<detail::server_session> const &s) {
  ...
  if (it != end(pimpl->sessions_)) {
    session_end_callback_ ? session_end_callback_.value()((*it)->id()) : 0;
